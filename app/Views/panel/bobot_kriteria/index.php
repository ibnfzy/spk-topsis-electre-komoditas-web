...
<section class="space-y-6">
    <div class="bg-white/80 backdrop-blur glass-panel border border-slate-200 rounded-2xl shadow-floating p-6 lg:p-8">
        <div class="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-6">
            ...
            <a href="<?= base_url('panel/bobot-kriteria/tambah'); ?>" class="inline-flex items-center justify-center px-4 py-2.5 rounded-xl bg-gradient-to-r from-primary to-primaryDark text-white font-medium shadow-lg shadow-primary/30 hover:shadow-xl hover:-translate-y-0.5 transition-all">
    ...
    <div class="bg-white/70 backdrop-blur glass-panel border border-slate-200 rounded-2xl shadow-floating">
        <div class="p-6 border-b border-slate-100 flex flex-col lg:flex-row lg:items-center lg:justify-between gap-4">
            <div class="relative w-full lg:max-w-sm">
                ...
                <input id="searchInput" type="search" placeholder="Cari bobot kriteria..." class="w-full rounded-xl border border-slate-200 bg-white/80 py-2.5 pl-10 pr-4 text-slate-700 placeholder:text-slate-400 focus:border-primary focus:ring-2 focus:ring-primary/20 transition-all" />
            </div>
            <div class="text-sm text-slate-500">
                <span id="tableMeta">Menampilkan bobot preferensi</span>
            </div>
        </div>
            ...
            <table class="min-w-full divide-y divide-slate-100">
                <thead class="bg-slate-50/60">
                    <tr>
                        ...
                        <th class="px-6 py-3 text-left text-xs font-semibold uppercase tracking-wider text-slate-500">Bobot</th>
...
<script>
    document.addEventListener('DOMContentLoaded', () => {
        const endpoint = '<?= base_url('panel/bobot-kriteria?format=json'); ?>';
        ...
        const renderRows = () => {
                    ...
                    <tr>
                        <td colspan="4" class="px-6 py-10 text-center text-slate-400">
                            <div class="flex flex-col items-center gap-3">
                                ...
                                <p class="font-medium">Belum ada data bobot.</p>
                                <p class="text-sm text-slate-400">Tambahkan bobot untuk menentukan prioritas kriteria.</p>
                            </div>
                        </td>
                    </tr>
                `;
            ...
            pageItems.forEach((item, index) => {
                ...
                row.innerHTML = `
                    ...
                    <td class="px-6 py-4 text-sm font-medium text-slate-700">${item.nama_kriteria ?? '-'}</td>
                    <td class="px-6 py-4 text-sm text-slate-500">${item.bobot ?? '-'}</td>
                    <td class="px-6 py-4">
                        <div class="flex justify-end gap-2">
                            <a href="<?= base_url('panel/bobot-kriteria'); ?>/${item.id}/edit" class="inline-flex items-center px-3 py-1.5 rounded-lg border border-primary/30 text-primary bg-primary/10 hover:bg-primary/20 transition-all text-sm font-medium">Edit</a>
            ...
            tableMeta.textContent = `Menampilkan ${pageItems.length} dari ${filtered.length} bobot`;
        ...
        const applyFilter = () => {
            ...
            filtered = dataset.filter(item => {
                return [item.nama_kriteria, item.bobot]
                    .filter(Boolean)
        ...
        const attachDeleteEvents = () => {
            document.querySelectorAll('.deleteBtn').forEach(btn => {
                btn.addEventListener('click', () => {
                    ...
                    if (!id) return;
                    if (!confirm('Yakin ingin menghapus bobot ini?')) return;

                    fetch(`<?= base_url('panel/bobot-kriteria'); ?>/${id}`, {
                        method: 'DELETE',
                        headers: { 'Accept': 'application/json' },
                    })
                        .then(res => res.json())
                        .then(() => {
                            toast('Bobot berhasil dihapus');
                ...
                <tr>
                    <td colspan="4" class="px-6 py-10 text-center">
                        <div class="flex flex-col items-center gap-3 text-slate-400 animate-pulse">
                            ...
                            <p class="text-sm">Memuat data bobot kriteria...</p>
                        </div>
                    </td>
                </tr>`;
