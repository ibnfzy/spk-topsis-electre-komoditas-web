...
<section class="space-y-10">
    <header
        class="rounded-3xl bg-gradient-to-br from-white via-white/90 to-slate-100/60 border border-white/80 shadow-floating p-8 lg:p-10">
        <div class="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-8">
            <div class="space-y-5 max-w-2xl">
                <div>
                    <p class="text-xs uppercase tracking-[0.4em] text-primary/70 font-semibold">Sistem Pendukung Keputusan</p>
                    <h1 class="mt-3 text-3xl lg:text-4xl font-semibold text-slate-900">Dashboard SPK Komoditas Tambak Air Payau
                    </h1>
                    <p class="mt-3 text-slate-600">Pantau ringkasan metrik utama dan jalankan analisis TOPSIS maupun ELECTRE
                        secara instan dari satu tempat.</p>
                </div>
            </div>
            <div
                class="rounded-3xl bg-white border border-slate-100 shadow-floating px-8 py-6 text-sm text-slate-600 space-y-3">
    ...
    <section class="grid gap-6 md:grid-cols-2 xl:grid-cols-4">
        ...
        <?php foreach ($statCards as $card): ?>
            <article
                class="group rounded-3xl bg-white/80 backdrop-blur glass-panel border border-slate-100 px-6 py-7 shadow-floating transition transform hover:-translate-y-1 hover:shadow-2xl">
                <div class="flex items-center justify-between gap-6">
                    <div>
                        ...
                        <p class="mt-4 text-3xl font-semibold text-slate-900">
                            <?= esc(number_format((float) ($card['value'] ?? 0))); ?></p>
                        <p class="mt-2 text-sm text-slate-500"><?= esc($card['description']); ?></p>
                    </div>
                    <span
                        class="inline-flex h-12 w-12 items-center justify-center rounded-2xl bg-primary/10 text-primary group-hover:bg-primary group-hover:text-white transition">
    ...
    <section class="grid gap-6 lg:grid-cols-[1.1fr,0.9fr]">
        <div
            class="rounded-3xl bg-white/90 backdrop-blur glass-panel border border-slate-100 shadow-floating p-8 space-y-8">
            ...
            <div class="grid gap-4 md:grid-cols-3">
                <button data-action="topsis"
                    class="dashboard-action group rounded-2xl border border-primary/20 bg-primary/10 px-5 py-6 text-left shadow-inner hover:bg-primary hover:text-white transition">
                    <span
                        class="inline-flex h-10 w-10 items-center justify-center rounded-xl bg-primary/20 text-primary group-hover:bg-white/20 group-hover:text-white transition">
                        <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5" fill="none" viewBox="0 0 24 24"
                            stroke="currentColor" stroke-width="1.5">
                            <path stroke-linecap="round" stroke-linejoin="round"
                                d="M3 3v7.5M3 3h7.5M3 3l7.5 7.5m0 0V21m0-10.5H21M10.5 10.5L21 21" />
                        </svg>
                    </span>
                    <span class="mt-4 block text-lg font-semibold">Hitung TOPSIS</span>
                    <span class="mt-2 block text-sm text-slate-500 group-hover:text-white/80">Lakukan peringkat berbasis
                        preferensi ideal.</span>
                </button>
                <button data-action="electre"
                    class="dashboard-action group rounded-2xl border border-amber-200 bg-amber-50/80 px-5 py-6 text-left shadow-inner hover:bg-amber-400 hover:text-slate-900 transition">
                    <span
                        class="inline-flex h-10 w-10 items-center justify-center rounded-xl bg-amber-100 text-amber-600 group-hover:bg-amber-200/60 group-hover:text-amber-900 transition">
                        <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5" fill="none" viewBox="0 0 24 24"
                            stroke="currentColor" stroke-width="1.5">
                            <path stroke-linecap="round" stroke-linejoin="round"
                                d="M4.5 19.5l3.75-3.75m0 0L12 18 19.5 6.75m-11.25 9L19.5 6.75M8.25 15.75L4.5 12" />
                        </svg>
                    </span>
                    <span class="mt-4 block text-lg font-semibold">Hitung ELECTRE</span>
                    <span class="mt-2 block text-sm text-slate-500 group-hover:text-slate-800/80">Analisis outranking untuk
                        dominasi alternatif.</span>
                </button>
                <button data-action="bandingkan"
                    class="dashboard-action group rounded-2xl border border-emerald-200 bg-emerald-50 px-5 py-6 text-left shadow-inner hover:bg-emerald-500 hover:text-white transition">
                    <span
                        class="inline-flex h-10 w-10 items-center justify-center rounded-xl bg-emerald-100 text-emerald-600 group-hover:bg-white/20 group-hover:text-white transition">
                        <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5" fill="none" viewBox="0 0 24 24"
                            stroke="currentColor" stroke-width="1.5">
                    ...
                    <span class="mt-2 block text-sm text-slate-500 group-hover:text-white/80">Temukan korelasi hasil TOPSIS dan
                        ELECTRE.</span>
                </button>
            </div>
            ...
            <div class="mt-6 grid gap-4 md:grid-cols-3">
                <?php foreach ($resultLinks as $link): ?>
                    <a href="<?= $link['href']; ?>"
                        ...
                        <span
                            class="inline-flex h-10 w-10 items-center justify-center rounded-xl bg-<?= esc($link['color']); ?>/10 text-<?= esc($link['color']); ?> group-hover:bg-<?= esc($link['color']); ?> group-hover:text-white transition">
                            <?= $link['icon']; ?>
                        </span>
                        <span class="mt-4 text-base font-semibold text-slate-900"><?= esc($link['label']); ?></span>
                        <span
                            class="mt-1 text-sm text-slate-500 group-hover:text-slate-600/90"><?= esc($link['description']); ?></span>
                    </a>
                <?php endforeach; ?>
            </div>
        </div>
...
<script>
    document.addEventListener('DOMContentLoaded', () => {
        ...
        const showAlert = (message, type = 'success') => {
            ...
            alertBox.className =
                `mt-6 rounded-2xl border px-4 py-3 text-sm ${type === 'success' ? 'border-emerald-200 bg-emerald-50 text-emerald-700' : 'border-rose-200 bg-rose-50 text-rose-600'}`;
            ...
            button.addEventListener('click', () => {
                ...
                fetch(url, {
                    ...
                    .then((payload) => {
                        ...
                        showAlert(`Permintaan ${action.toUpperCase()} berhasil dikirim. Silakan cek hasilnya.`,
                            'success');
                    })
                    .catch((error) => {
