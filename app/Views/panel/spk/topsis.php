...
<script>
    document.addEventListener('DOMContentLoaded', () => {
        ...
        const dataEndpoint = '<?= base_url('panel/spk/topsis/data'); ?>';
        const calculateEndpoint = '<?= base_url('panel/spk/topsis/hitung'); ?>';
        ...
        const getPreferenceValue = (row) => {
            const rawValue = row?.nilai_preferensi ?? row?.nilai_pref ?? row?.nilai;
        ...
        const renderTable = (rows = []) => {
            ...
            rows.forEach((row, index) => {
                ...
                const preference = getPreferenceValue(row);
                ...
                tr.innerHTML = `
                    ...
                    <td class="px-6 py-4 text-sm text-slate-500">${formatNumber(preference)}</td>
            ...
            fetch(dataEndpoint, {
                ...
                .then((payload) => {
                    if (payload?.status !== 'success') {
                        throw new Error(payload?.message || 'Tidak dapat menampilkan data TOPSIS.');
                    }
        ...
        const runCalculation = () => {
            ...
            fetch(calculateEndpoint, {
                ...
                .then((payload) => {
                    ...
                    hydrate(payload.data || {});
                    showAlert(payload?.message || 'Hasil TOPSIS berhasil diperbarui.', 'success');
                })
                .catch((error) => {
